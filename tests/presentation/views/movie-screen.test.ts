...
function renderMovieScreen(locale: Locale) {
...
describe('MovieScreen', () => {
  ...
  it('renders the documented placeholder content in English', () => {
    // Arrange
    const wrapper = renderMovieScreen('en')

    // Assert
    expect(wrapper.findComponent(EmptyState).exists()).toBe(true)
    expect(wrapper.findComponent(Film).exists()).toBe(true)
  ...
  it('renders the documented placeholder content in French', () => {
    // Arrange
    const wrapper = renderMovieScreen('fr')
  ...
  it('uses shared translation bindings and contains no hardcoded placeholder copy', () => {
    ...
    const sourceFile = readFileSync(
      resolve(process.cwd(), 'src/presentation/views/movie-screen.vue'),
      'utf8',
    )
