...
function renderStatsScreen(locale: Locale) {
  return mount(StatsScreen, {
    global: {
...
describe('StatsScreen', () => {
  ...
  it('renders the documented placeholder content in English', () => {
    // Arrange
    const wrapper = renderStatsScreen('en')

    // Assert
    expect(wrapper.findComponent(EmptyState).exists()).toBe(true)
    expect(wrapper.findComponent(ChartColumn).exists()).toBe(true)
  ...
  it('renders the documented placeholder content in French', () => {
    // Arrange
    const wrapper = renderStatsScreen('fr')
  ...
  it('uses shared translation bindings and contains no hardcoded placeholder copy', () => {
    ...
    const sourceFile = readFileSync(
      resolve(process.cwd(), 'src/presentation/views/stats-screen.vue'),
      'utf8',
    )
