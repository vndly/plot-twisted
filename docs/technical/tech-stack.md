...
- **vue-i18n v11** — Official Vue internationalization library. Composition API mode (`legacy: false`) with pre-compiled messages via `@intlify/unplugin-vue-i18n`. Provides `$t()` in templates and `useI18n()` in `<script setup>`.
