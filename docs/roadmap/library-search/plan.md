...
2. [ ] **Run affected Library tests**: Run `npm run test -- tests/domain/library-search.schema.test.ts tests/domain/library-search.logic.test.ts tests/application/use-library-search.test.ts tests/application/use-library-entries.test.ts tests/presentation/components/library/library-search-bar.test.ts tests/presentation/views/library-screen.test.ts`.
...
4. [ ] **Manual responsive check**: Run `npm run dev`, open `/library`, and verify at 390px and 1024px viewport widths that search placement, clear-button target, sticky controls, focus behavior, and empty-state CTA behavior match the requirements.
