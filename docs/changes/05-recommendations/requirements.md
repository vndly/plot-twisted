...
Enhance content discovery by providing personalized "If you liked X" recommendations based on the user's library entries, with a fallback to trending and popular content when no library data is available.
...
- **02-home**: Recommendations are integrated into the home screen discovery flow.
- **03-library**: Requires access to the user's library entries (ratings, watch dates) for seeding.
...
- Deduplication of recommendations across multiple seeds.
- Filtering out recommendations that are already present in the user's library.
...
- "Similar titles" on the detail page (covered by 02-home "Out of Scope" but could be a future extension).
...
| :---- | :---------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------- |
| RE-01 | Seed Selection Logic    | The app SHALL select up to 5 seed entries from the library, prioritizing highest `rating` first, then most recent `addedAt` or `watchDates` for ties or unrated items. | P0       |
...
| RE-03 | Deduplication           | The app SHALL deduplicate recommendations across all seed results using the provider `id` to ensure no title appears twice in the recommendations sections.            | P0       |
...
| RE-05 | Labeled Sections        | Each recommendation group SHALL be displayed in a carousel labeled "Because you liked {title}", where `{title}` is the title of the seed entry.                        | P0       |
...
- **UI Responsiveness**: The home screen SHALL remain interactive while recommendations are loading.
...
- Labels SHALL use a consistent heading style (e.g., `h3` or `h2` with subtitle).
- The transition between loading skeleton and content SHALL be smooth.

## Risks & Assumptions

- **Risk**: API Rate Limits. Fetching 5 recommendation sets in parallel plus trending/popular might hit TMDB limits. _Mitigation_: Use existing retry logic and ensure only necessary calls are made.
...
- [ ] Up to 5 seed entries are selected from the library (highest rated first).
