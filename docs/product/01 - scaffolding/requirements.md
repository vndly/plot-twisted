...
- [ ] [SC-05] Sidebar renders the `app.title` branding block plus Home, Calendar, Library, and Settings with their mapped icons and translated `nav.*` labels
- [ ] [SC-05, SC-06] Sidebar and bottom nav render exactly 4 primary items in the same order; Recommendations, Stats, and detail routes are absent from navigation in this phase
