...
- **Testing coverage**: Plan must include a testing phase with test files mapped to the components or logic they validate. Flag if testing is absent or deferred without justification.
- **Dependency management**: Steps that install packages must specify version ranges (e.g., `vue-router@^4`). Flag any dependency added without a version constraint.
